...
def get_graph_edit_distance(G: nx.Graph, G_rewired: nx.Graph):
    return nx.graph_edit_distance(G, G_rewired)

def get_laplac_spec_dist(G: nx.Graph, G_rewired: nx.Graph, p=2):
    L1 = nx.laplacian_matrix(G).toarray()
    L2 = nx.laplacian_matrix(G_rewired).toarray()
...
#Function to go through a rewired dataset and calculate average comparison metrics
def comparison_metrics(G: nx.Graph, G_rewired: nx.Graph, G_name: str):
    # # if they have the same # of graphs
    # if len(self.G) != len(self.G_rewired):
    #     print("Not the same dataset")
    # else:
    ...
    metrics["Graph Edit Distance"](get_laplac_spec_dist(G, G_rewired))
